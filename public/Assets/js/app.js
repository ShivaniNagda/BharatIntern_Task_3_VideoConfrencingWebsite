...
var AppProcess = (function () {
  ...
  async function loadAudio() {
    try {
      ...
      console.log(e);
      alert("Audio permission has not been granted or there is an issue with audio.");
    }
  }
  ...
  //  Function Start--Rem ove Media Sender---------------------

  function removeMediaSenders(rtp_senders) {
    for (var con_id in peers_connection_ids) {
      if (rtp_senders[con_id] && connection_status(
        peers_connection[con_id])) {
  ...
  //  Function Start----Remove Video Stram-------------------
  ...
  async function videoProcess(newVideoState) {
    ...
    if (newVideoState == video_states.Camera) {
      ...
      $("#ScreenShareOnOff").html
        ('<span class = "material-icons text-success">present_to_all</span><div">Present Now</div>');
...
var MyApp = (function () {
  ...
  function event_process_for_signaling_server() {
    ...
    socket.on("showFileMessage", function(data){
      ...
      attachFileAreaForOther.innerHTML += "<div class='left-align' style='display:flex; align-items:center;'><img src = 'public/assets/image/other.jpg' style='height:40px; width:40px;' class='caller-image circle'><div style ='font-weight:600;margin:0 5px;'>"+data.username+"</div>:<div><a style='color:#007bff;' href='"+data.filePath+"' download"+data.fileName+"</a></div></div><br/>";
    ...
    socket.on("SDPProcess", async function (data) {
      await AppProcess.processClientFunc(
        data.message, data.from_connid);
    ...
    socket.on("showChatMessage", function (data) {
      ...
      var div = $("<div>").html(
        "<span class ='font-weight-bold mr-3' style='color:black' font-size:'1.2rem' >"
        + data.from + "</span>" + lTime + "</br>" + data.message );
      $("#messages").append(div);
    });
  }
  ...
  function eventHandeling() {
    $("#btnsend").on("click", function () {
      ...
      var div = $("<div>").html(
        "<span class ='font-weight-bold mr-3' style='color:black' font-size:'1.2rem' margin='15px'>"
        + user_id + "</span>" + " " + lTime + "</br>" + msgData );
      $("#messages").append(div);
  ...
  function addUser(other_user_id, connId, userNum) {
    ...
    $(".in-call-wrap-up").append(' <div class="in-call-wrap d-flex justify-content-between align-items-center mb-3" id="participant_'+connId+'"> <div class="participant-img-name-wrap display-center cursor-pointer"> <div class="participant-img"> <img src="public/Assets/images/other.jpg" alt="" class="border border-secondary" style="height: 40px; width: 40px; border-radius: 50%;"> </div> <div class="participant-name ml-2"> '+other_user_id+'</div> </div> <div class="participant-action-wrap display-center "> <div class="participant-action-dot display-center mr-2 cursor-pointer"> <span class="material-icons">more_vert</span> </div> <div class="participant-action-wrap display-center cursor-pointer"> <div class="participant-action-pin display-center mr-2 cursor-pointer"> <span class="material-icons">push_pin</span> </div> </div> </div> ');
...
attachFileArea.innerHTML += "<div class='left-align' style='display:flex; align-items:center;'><img src = 'public/assets/image/other.jpg' style='height:40px; width:40px;' class='caller-image circle'><div style ='font-weight:600;margin:0 5px;'>"+user_id+"</div>:<div><a style='color:#007bff;' href='"+attachFilePath+"' download"+attachFileName+"</a></div></div><br/>";
...
async function  startRecording(){
 ...
 const stream = new MediaStrem([...screenStream.getTracks(), ...audioStream.getTracks()])
